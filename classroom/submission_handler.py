...
def summarize_attachment_context(
    attachment_urls: list[str], material_texts: list[str], max_chars: int = 2200
) -> str:
    lines: list[str] = []
    if attachment_urls:
...
def extract_doc_section_prompts(doc_text: str, max_fields: int = 24) -> list[str]:
    ...
    for raw in (doc_text or "").splitlines():
        ...
        prompt_like = bool(
            ...
            or re.search(
                r"\b(what|why|how|describe|explain|list|choose|brief summary|source type|free response)\b",
                lower,
            )
        )
        if not prompt_like:
...
def _clean_answer(text: str) -> str:
    ...
    out = re.sub(
        r"^\s*\[\s*answer\s*\d*\s*\]\s*", "", out, flags=re.IGNORECASE | re.MULTILINE
    )
...
def match_answers_to_fields(
    ...
    for fi, fld in enumerate(fields):
        ...
        if best_idx >= 0 and best_score >= 0.15:
            ...
            if best_score < 0.3:
                logger.warning(
                    "  SmartFill AMBIGUOUS MATCH: field '%s' matched with score %.2f (low confidence)",
                    fld.nearby_text[:60],
                    best_score,
                )
        else:
...
async def _human_type(
    page: Page, text: str, min_ms: int = 20, max_ms: int = 48
) -> None:
        ...
        for wi, word in enumerate(words):
            ...
            if len(buf) >= burst:
                await page.keyboard.type(
                    "".join(buf), delay=random.randint(min_ms, max_ms)
                )
                buf.clear()
                burst = random.randint(2, 5)
...
async def _fill_single_field(
    ...
    try:
        ...
        logger.warning(
            "  SmartFill: could not click field %d (%s): %s",
            field_num,
            fld.nearby_text[:60],
            exc,
        )
...
async def _fallback_fill_first_field(
    ...
    fld = fields[0]
    logger.warning(
        "  SmartFill FALLBACK: dumping combined answer into first field (%s)",
        fld.nearby_text[:60],
    )
...
async def smart_fill_fields(
    ...
    for i, f in enumerate(fields):
        logger.info(
            "  SmartFill: field %d [%s] nearby='%s'", i, f.tag, f.nearby_text[:80]
        )

    if len(fields) > 10:
        logger.warning(
            "  SmartFill AMBIGUOUS: detected %d fields - many fields may indicate incorrect page or template. Verify assignment submission type.",
            len(fields),
        )
    ...
    for idx, (fld, answer_text, score) in enumerate(matches):
        if not answer_text.strip():
            logger.warning(
                "  SmartFill: empty answer for field %d (%s), skipping",
                idx,
                fld.nearby_text[:60],
            )
        ...
        ok = await _fill_single_field(
            page, fld, answer_text, debug_dir=debug_dir, field_num=idx
        )
        if ok:
            ...
            result.failed_count += 1
            logger.warning(
                "  SmartFill: FAILED field %d (%s)", idx, fld.nearby_text[:60]
            )
...
async def fill_detected_fields(
    ...
    det_fields = [
        DetectedField(
            index=i,
            field_id=f.locator_css.split('"')[1]
            if '"' in f.locator_css
            else f"legacy-{i}",
            tag=f.tag,
            role=f.role,
    ...
    for idx, (fld, answer_text, score) in enumerate(matches):
        ...
        try:
            ...
            filled += 1
            logger.info(
                "  fill_detected_fields: filled %d/%d score=%.2f",
                idx + 1,
                len(fields),
                score,
            )
...
async def fill_doc_sections(
    ...
    filled = 0
    for idx, (prompt, matched_question, answer_text, score) in enumerate(
        matches, start=1
    ):
        if not answer_text.strip():
            continue
        ...
        if ok:
            ...
            logger.warning(
                "  fill_doc_sections: could not place answer under prompt: %s",
                prompt[:60],
            )
    return filled
