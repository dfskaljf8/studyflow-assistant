...
from classroom.scanner import (
    Assignment,
    SUBMISSION_TYPE_TYPED,
...
async def _open_and_verify_assignment(
    ...
    for attempt in range(1, 3):
        ...
        if attempt == 1:
            logger.warning(
                "  Paste: assignment verification failed, retrying navigation once"
            )
...
async def _find_first_visible(
    page: Page, selectors: list[str], timeout_ms: int, context: str = ""
) -> Locator | None:
    if not selectors:
        return None
    ...
    for selector in selectors:
        ...
        if count > 1:
            ...
            if visible_count > 1:
                logger.warning(
                    "AMBIGUOUS DOM: selector '%s' has %d visible matches (context: %s) - using first match",
                    selector,
                    visible_count,
                    context,
                )
...
async def paste_draft(
    ...
    submission_type = assignment.submission_type or SUBMISSION_TYPE_UNKNOWN
    logger.info(
        "  Paste: routing based on detected submission type: %s", submission_type
    )

    if submission_type == SUBMISSION_TYPE_FILE_UPLOAD:
        logger.warning(
            "  Paste: assignment '%s' requires file upload - skipping (no explicit file path provided)",
            assignment.title,
        )
        assignment.delivery_method = "skipped"
        assignment.delivery_details = "file_upload_requires_explicit_path"
        return False
    ...
    try:
        ...
        if (
            submission_type == SUBMISSION_TYPE_TYPED
            or submission_type == SUBMISSION_TYPE_UNKNOWN
        ):
            ...
            if delivered_via_fields:
                return True
            if submission_type == SUBMISSION_TYPE_TYPED:
                assignment.delivery_method = "failed"
                assignment.delivery_details = "typed_response_field_fill_failed"
                logger.warning(
                    "Typed response delivery failed for: %s", assignment.title
                )
                return False

        if (
            submission_type == SUBMISSION_TYPE_GOOGLE_DOC
            or submission_type == SUBMISSION_TYPE_UNKNOWN
        ):
            ...
            if delivered_via_doc:
                return True
            if submission_type == SUBMISSION_TYPE_GOOGLE_DOC:
                assignment.delivery_method = "failed"
        ...
        if (
            ...
            if delivered_via_ap:
                return True
            if submission_type == SUBMISSION_TYPE_AP_CLASSROOM:
                assignment.delivery_method = "failed"
                assignment.delivery_details = "ap_classroom_delivery_failed"
                logger.warning("AP Classroom delivery failed for: %s", assignment.title)
                return False

        assignment.delivery_method = "failed"
        assignment.delivery_details = "doc_and_ap_delivery_failed"
        logger.warning(
            "All delivery methods failed for: %s (submission_type: %s)",
            assignment.title,
            submission_type,
        )
        return False
