...
def process_assignment_node(state: WorkflowState) -> dict:
    ...
    try:
        ...
        if not draft:
            draft_validation_error = "empty_draft"
        elif len(draft) < MIN_DRAFT_LENGTH:
            draft_validation_error = f"draft_too_short_{len(draft)}_chars"
        elif (
            "error" in (draft[:200] or "").lower()
            and "generating" in (draft[:200] or "").lower()
        ):
            draft_validation_error = "draft_contains_error"

        if draft_validation_error:
            logger.warning(
                "  Step 2 VALIDATION FAILED: %s - skipping paste step (Step 3/5) for: %s",
            ...
            _mark_assignment_state(
                ...
                a,
                status="failed",
                delivery_method="skipped",
                delivery_details=f"draft_validation_failed_{draft_validation_error}",
                pasted=False,
            )
            processed.append(
                {
